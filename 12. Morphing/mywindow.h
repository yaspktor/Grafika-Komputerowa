...
private:
    ...
    QPushButton *animacja;
    //suwak do zmiany obrazka
    QSlider *slider;
    //label
    QLabel *label;
    // Grupa przyciskow
    ...
    std::vector<std::pair<int, int> > PunktyTrojkataNaPustym;
    //dodanie 3 punktow do trojkata

    std::vector<std::pair<int, int> > punktyTrojkataObrazek2;

    std::vector<std::pair<int, int> > punktyTrojkataObrazek1;
    ...
    void animacjaTr();
    void WypelnijTrojkat(std::vector<std::pair<int, int> > punkty, double t);
    void refreshBoard1();
    void refreshBoard2();
