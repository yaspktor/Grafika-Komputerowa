...
void MyWindow::rysujKrzywa(std::vector<std::pair<int, int> > punkty, int n, bool odcinki)
{
   ...
   for (int i = 0; i < n; i++)
   {
        t = (float)i / n;
        t2 = t + 1/(float)n;
        Px = ((float)1/6)* (-pow((t), 3) + 3*t*t - 3 * t + 1) * punkty[0].first + ((float)1/6)* (3*t*t*t - 6 * t*t +4)
                * punkty[1].first + ((float)1/6)* (-3 * pow(t, 3) + 3*t*t + 3 * t + 1)* punkty[2].first
                + ((float)1/6)* (t*t*t) * punkty[3].first;
        Py = ((float)1/6)* (-pow((t), 3) + 3*t*t - 3 * t + 1) * punkty[0].second + ((float)1/6)* (3*t*t*t - 6 * t*t +4)
                * punkty[1].second + ((float)1/6)* (-3 * (pow(t, 3)) + 3*t*t + 3 * t + 1)* punkty[2].second
                + ((float)1/6)* (t*t*t) * punkty[3].second;
        Qx = ((float)1/6)* (-pow((t2), 3) + 3*t2*t2 - 3 * t2 + 1) * punkty[0].first + ((float)1/6)* (3*t2*t2*t2 - 6 * t2*t2 +4)
                * punkty[1].first + ((float)1/6)* (-3 * pow(t2, 3) + 3*t2*t2 + 3 * t2 + 1)* punkty[2].first
                + ((float)1/6)* (t2*t2*t2) * punkty[3].first;
        Qy = ((float)1/6)* (-pow((t2), 3) + 3*t2*t2 - 3 * t2 + 1) * punkty[0].second + ((float)1/6)* (3*t2*t2*t2 - 6 * t2*t2 +4)
                * punkty[1].second + ((float)1/6)* (-3 * pow(t2, 3) + 3*t2*t2 + 3 * t2 + 1)* punkty[2].second
                + ((float)1/6)* (t2*t2*t2) * punkty[3].second;
        rysujLinie(Px, Py, Qx, Qy, 255);
        //qDebug() << "narysowano1:  x:" << Px << " y: "<<Py;
        //qDebug() << "narysowano2:  x:" << Qx << " y: "<<Qy;
    ...
    for (int i = 0; i < (int)punkty.size(); i++) {
        if (abs(punkty[i].first - x) < 4 && abs(punkty[i].second - y) < 4) {
            ...
            if (podKursorem) {
				...
				podKursorem = false;
                // zeby nie było przerw między krzywimi 
                //refreshBoard(punkty);
				refreshBoard(punkty);
			}
...
void MyWindow::fragmentKrzywej(std::vector<std::pair<int, int> > punkty)
{
    ...
    for (int i = 0; i < (int)punkty.size(); i++)
    {
        ...
        if (narysowanePunkty >= 3) // Zmienia warunek na narysowanePunkty zamiast i
        {
            qDebug() << "narysowano " << narysowanePunkty;
            rysujKrzywa(punktyDoKrzywej, n, odcinki);
            punktyDoKrzywej.erase(punktyDoKrzywej.begin());
        }
        // Usuwa zbędne warunki else if (i > 3)
        narysowanePunkty++; // Zwiększa liczbę narysowanych punktów
    }
}
...
void MyWindow::mousePressEvent(QMouseEvent *event)
{
    ...
    if (event->buttons() & Qt::RightButton)
    {
            ...
            //
            double minDist = 100000; // ustawiamy początkową wartość na dużą wartość
            int index = 0;
            for (int i = 0; i < (int)punkty.size(); i++) {
